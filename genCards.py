...
import os
with open(f'{os.path.dirname(__file__)}\\hsServer\\gameModules\\Cards4server.py','w') as f:
    f.write(importStr+CardsStr)
importStr = importStr.replace('Server','Client')
with open(f'{os.path.dirname(__file__)}/hsClient/gameModules/Cards4client.py','w') as f:
    f.write(importStr+CardsStr)